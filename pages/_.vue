  ...
  <v-row justify="center" align="center" class="pa-0">
    <v-app-bar v-if="isMobile" flat fixed dense style="top:56px;">
      ...
      <v-btn
        ...
        class="ml-1"
        nuxt
        @click.stop="toggleTocDrawer()"
      >
        <v-icon>mdi-text-search</v-icon>
      </v-btn>
    </v-app-bar>
    <v-col cols="12" :class="{'pa-2': true, 'pb-12': true, 'pt-12': isMobile, 'pt-6': !isMobile}">
      <v-alert :value="fallback" type="error" text outlined dismissible>
        {{ $t('slug.fallback', [locale.name, defaultLocale.name ]) }}
      </v-alert>
      <v-card v-if="!!article" flat tile class="bg-0" key="card">
    ...
    <resizable-drawer side="right" id="toc" :dark="darkmode">
...
export default {
  ...
  computed: {
    ...
    defaultLocale() {
      return this.$i18n.locales.filter(i => i.code === this.$i18n.defaultLocale)[0]
    },
    github() {
